...
export function buildToolFilter(config: {
  ...
  if (allow === undefined) {
    ...
    for (const entry of allow) {
      ...
      const matches = expand(entry, 'MEALIE_ALLOW_TOOLS');
      if (matches.length === 0) {
        throw new ToolFilterError(
          `MEALIE_ALLOW_TOOLS: no tool matches "${entry}". ` +
      ...
      if (survivors.length === 0) {
        if (entry.endsWith('*')) {
          ...
          console.error(
            `mealie-mcp: MEALIE_ALLOW_TOOLS: "${entry}" matches only write ` +
              'tools, which MEALIE_READ_ONLY suppresses — it contributes nothing.'
          );
        ...
        throw new ToolFilterError(
          `MEALIE_ALLOW_TOOLS: "${entry}" is a write tool, but MEALIE_READ_ONLY ` +
            'is set — it is never registered. Remove it from MEALIE_ALLOW_TOOLS, ' +
            `or unset MEALIE_READ_ONLY. Available in read-only mode: ${[...READ_TOOLS].sort().join(', ')}.`
        );
      }
  ...
  for (const entry of deny ?? []) {
    ...
    const matches = expand(entry, 'MEALIE_DENY_TOOLS');
    if (matches.length === 0) {
      throw new ToolFilterError(
        `MEALIE_DENY_TOOLS: no tool matches "${entry}". Valid tools: ${catalogueList()}.`
      );
    }
  ...
  if (selected.size === 0) {
    throw new ToolFilterError(
      suppressedByReadOnly
        ? 'MEALIE_ALLOW_TOOLS selects only write tools, but MEALIE_READ_ONLY is ' +
            'set — the server would start with an empty tool list.'
        : 'MEALIE_ALLOW_TOOLS/MEALIE_DENY_TOOLS leave no tools registered — the ' +
