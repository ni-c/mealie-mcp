...
`preview_recipe_url` and the image import hand a URL or a payload to Mealie,
which retrieves it from inside its own network. URLs are therefore restricted to
`http` and `https` — zod's `.url()` alone would accept `file:`, `javascript:`
and `data:` — and loopback, private-range, link-local and
`.lan`/`.internal`/`.local` hosts are refused, since no recipe lives there. This
is not a complete SSRF defence and cannot be: the name is resolved by Mealie,
and a public name can point at a private address. The real boundary is Mealie's
own network egress.
