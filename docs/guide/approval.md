...
execute “merge B into A”.

## What the dialog contains

Ids, counts and flags. Never a recipe name, a tag or a comment — a recipe imported
from a website carries whatever that site wrote, it stays in the database
afterwards, and the prompt is read by a model at the exact moment it is deciding.

```
This will merge the food 3f2a… into 91c7…

The food 3f2a… is deleted and every reference to it is rewritten. This cannot be
undone.
```
...
`ELICITATION` has no `MEALIE_` in front of it, so one
