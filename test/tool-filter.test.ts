...
describe('selecting tools', () => {
  ...
  it('removes a whole family with a prefix pattern', async () => {
    const names = await toolNames({ denyTools: 'create_*' });
    expect(names.some((n) => n.startsWith('create_'))).toBe(false);
    expect(names).toHaveLength(
      ALL_TOOLS.length - ALL_TOOLS.filter((t) => t.startsWith('create_')).length
    );
  });
  ...
  it('subtracts the deny list from the allow list', async () => {
    expect(
      await toolNames({
        allowTools: 'get_about,get_cookbook',
        denyTools: 'get_cookbook',
      })
    ).toEqual(['get_about']);
  });
...
describe('together with read-only mode', () => {
  ...
  it('lets a pattern cover write tools without failing', async () => {
    ...
    expect(
      await toolNames({ ...readOnly, allowTools: 'essential,add_*' })
    ).toEqual(
      ESSENTIAL_TOOLS.filter((t) =>
        (READ_TOOLS as readonly string[]).includes(t)
      ).sort()
